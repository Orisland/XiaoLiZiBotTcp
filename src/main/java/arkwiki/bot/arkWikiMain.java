	...
	public static void receiveGroupMessages(String data){
		...
		try{
			...
			if (msg.indexOf("方舟") == 0){
				...
				String juse = msgSplit[1];
				if (msgSplit.length == 3 && msgSplit[2].equals("1")){
						 base64 = doc.docbase64(juse,1);
				}
				else {
					base64 = doc.docbase64(juse,0);
				}
				String pic = "[pic:"+base64+"]";
				Core.sendGroupMessagesPicText(selfQQ,fromGroup,pic,0);
			}
